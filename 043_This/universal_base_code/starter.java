...
class starter {
	...
	for(c=0; c<100; c++){
		...
		x[c].getStuffie();
		x[c].getStuffied();
		System.out.println(x[c].getStuffie()+ " be his name, and he be " +x[c].getStuffied()+" years old");
		if(x[0].isSameName(x[c].getStuffie()) == true){
			d++;
		}
	}
	System.out.println(x[0].getStuffie() +" appears " +d+ " times");
	}
}